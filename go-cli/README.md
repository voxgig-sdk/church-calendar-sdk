...
AQL-driven command-line client **and** interactive REPL for the ChurchCalendar
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/church-calendar-cli)
make build

# 2. See usage (words, entities, env vars)
./church-calendar-cli --help
...
./church-calendar-cli
church-calendar> list calendar
church-calendar> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/church-calendar-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/church-calendar-cli list calendar
   ```
...
Run with no arguments to open a REPL (prompt `church-calendar>`). Each line is
evaluated as its own AQL expression:

```text
$ ./church-calendar-cli
church-calendar> list calendar
church-calendar> /help
church-calendar> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `calendar`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/church-calendar-cli`. |
...
(`./church-calendar-cli <expr>`) and interactively (the REPL), and expressions compose the
