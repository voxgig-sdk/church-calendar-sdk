// Command ChurchCalendar-cli is a CLI + REPL for the ChurchCalendar SDK,
...
const prompt = "church-calendar"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "calendar"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CHURCH_CALENDAR_APIKEY (api key), CHURCH_CALENDAR_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CHURCH_CALENDAR_APIKEY carries the API key and
	// CHURCH_CALENDAR_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CHURCH_CALENDAR_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CHURCH_CALENDAR_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewChurchCalendarSDK(opts)
