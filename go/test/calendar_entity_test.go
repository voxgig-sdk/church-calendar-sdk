...
func TestCalendarEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Calendar(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Calendar(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CHURCHCALENDAR_TEST_CALENDAR_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		calendarRef01Match := map[string]any{
			"locale": setup.idmap["locale01"],
		}

		calendarRef01ListResult, err := calendarRef01Ent.List(calendarRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, calendarRef01ListOk := calendarRef01ListResult.([]any)
		if !calendarRef01ListOk {
...
func calendarBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CHURCHCALENDAR_TEST_CALENDAR_ENTID")
