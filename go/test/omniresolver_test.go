...
import (
	...
	omni "github.com/voxgig-sdk/church-calendar-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ChurchCalendarError); is {
				out := map[string]any{
					"name":    "ChurchCalendarError",
...
func omniCtx(arg any, client *sdk.ChurchCalendarSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ChurchCalendarSDK); is {
			client = live
