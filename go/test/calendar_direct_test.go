...
func TestCalendarDirect(t *testing.T) {
	t.Run("direct-list-calendar", func(t *testing.T) {
		setup := calendarDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-calendar", _mode); _shouldSkip {
			if _reason == "" {
...
type calendarDirectSetupResult struct {
	client *sdk.ChurchCalendarSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func calendarDirectSetup(mockres any) *calendarDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		return &calendarDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &calendarDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
