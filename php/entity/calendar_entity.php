...
// ChurchCalendar SDK Calendar entity
...
class CalendarEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CalendarEntity($this->_client, $opts);
    }

    /**
     * @param Calendar|array $args Calendar data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ChurchCalendarHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Calendar|array The current Calendar data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Calendar fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ChurchCalendarHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Calendar fields).
     */
     ...
     *
     * @param CalendarLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CalendarLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Calendar|array The loaded Calendar as an assoc-array at the
     *   SDK boundary; throws ChurchCalendarError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ChurchCalendarHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param CalendarListMatch|array|null $reqmatch Match filter (any subset
     *   of Calendar fields) as an assoc-array; CalendarListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Calendar[]|array A list of Calendar items as assoc-arrays at
     *   the SDK boundary; throws ChurchCalendarError on failure (item-5 convention).
     */
