# ChurchCalendar SDK test runner
...
class ChurchCalendarTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ChurchCalendarTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ChurchCalendarTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ChurchCalendarTestRunner.getenv("CHURCH_CALENDAR_TEST_LIVE")
        override = ChurchCalendarTestRunner.getenv("CHURCH_CALENDAR_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ChurchCalendarTestRunner.getenv(key)
        ...
        explain = ChurchCalendarTestRunner.getenv("CHURCH_CALENDAR_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ChurchCalendarTestRunner._test_control is not None:
            return ChurchCalendarTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ChurchCalendarTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ChurchCalendarTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ChurchCalendarTestRunner.load_test_control()
...
def load_env_local():
    ChurchCalendarTestRunner.load_env_local()


def env_override(m):
    return ChurchCalendarTestRunner.env_override(m)


def entity_data(v):
    return ChurchCalendarTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ChurchCalendarTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ChurchCalendarTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ChurchCalendarTestRunner.load_test_control()


def live_client_options():
    return ChurchCalendarTestRunner.live_client_options()


def live_delay_ms():
    return ChurchCalendarTestRunner.live_delay_ms()
