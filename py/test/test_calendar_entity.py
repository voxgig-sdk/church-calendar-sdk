...
class TestCalendarEntity:
    ...
    def test_should_stream(self):
        ...
        base = ChurchCalendarSDK.test(seed, None)
        seen = list(base.Calendar(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Calendar(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CHURCH_CALENDAR_TEST_CALENDAR_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        calendar_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.calendar")))
        calendar_ref01_data = None
        if len(calendar_ref01_data_raw) > 0:
...
def _calendar_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CHURCH_CALENDAR_TEST_CALENDAR_ENTID"))
    ...
    if env.get("CHURCH_CALENDAR_TEST_LIVE") == "TRUE":
        ...
        client = ChurchCalendarSDK(helpers.to_map(merged_opts))

    _live = env.get("CHURCH_CALENDAR_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("CHURCH_CALENDAR_TEST_EXPLAIN") == "TRUE",
        "live": _live,
