...
from churchcalendar_sdk.churchcalendar_types import (
    Calendar,
    CalendarLoadMatch,
    CalendarListMatch,
)
...
class CalendarEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CalendarEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Calendar:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Calendar().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Calendar]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Calendar().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
