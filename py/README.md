# ChurchCalendar Python SDK

The Python SDK for the ChurchCalendar API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Calendar(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.ChurchCalendar(None).load(
    {"id": "test01"}, None
)
...
```
py/
├── churchcalendar_sdk.py         -- Main SDK module
...
The main module (`churchcalendar_sdk`) exports the SDK class.
