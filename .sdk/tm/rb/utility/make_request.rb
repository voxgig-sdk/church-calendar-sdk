...
module ChurchCalendarUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ChurchCalendarResponse.new({})
    ...
    if fetch_err
      ...
      response = ChurchCalendarResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ChurchCalendarResponse.new(fetched)
    else
