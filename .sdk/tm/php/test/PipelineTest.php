...
// ChurchCalendar SDK pipeline test
//
...
{
    private static function utility(): ChurchCalendarUtility
    {
        return new ChurchCalendarUtility();
    }
    ...
    private static function ctx(array $over = []): ChurchCalendarContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ChurchCalendarContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ChurchCalendarOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ChurchCalendarError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ChurchCalendarOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ChurchCalendarMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ChurchCalendarOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ChurchCalendarMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ChurchCalendarOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ChurchCalendarMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ChurchCalendarSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ChurchCalendarMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ChurchCalendarResponse([]),
            'result' => new ChurchCalendarResult([]),
        ]);
        [, $err] = ChurchCalendarMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ChurchCalendarSpec([]),
            'response' => null,
            'result' => new ChurchCalendarResult([]),
        ]);
        [, $err] = ChurchCalendarMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ChurchCalendarSpec(['step' => 's']),
            'response' => new ChurchCalendarResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ChurchCalendarResult([]),
        ]);
        [, $err] = ChurchCalendarMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ChurchCalendarSpec(['step' => 's']),
            'response' => new ChurchCalendarResponse(self::resp(200, ['v' => 1])),
            'result' => new ChurchCalendarResult([]),
        ]);
        [, $err] = ChurchCalendarMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ChurchCalendarSpec(['step' => 's']),
            'response' => new ChurchCalendarResponse(self::resp(200, ['v' => 2])),
            'result' => new ChurchCalendarResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ChurchCalendarMakeResponse::call($ctx);
    ...
    {
        $preset = new ChurchCalendarResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ChurchCalendarSpec([]),
            'response' => new ChurchCalendarResponse([]),
            'result' => new ChurchCalendarResult([]),
        ...
        [$got, $err] = ChurchCalendarMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ChurchCalendarResult([])]);
        [, $err] = ChurchCalendarMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ChurchCalendarSpec([]), 'result' => null]);
        [, $err] = ChurchCalendarMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ChurchCalendarSpec(['step' => 's']),
            'result' => new ChurchCalendarResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ChurchCalendarOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ChurchCalendarMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ChurchCalendarSpec(['step' => 's']),
            'result' => new ChurchCalendarResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ChurchCalendarOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ChurchCalendarMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ChurchCalendarResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ChurchCalendarSpec([]),
            'result' => new ChurchCalendarResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ChurchCalendarMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ChurchCalendarMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ChurchCalendarError('boom', 'boom');
        $utility->fetcher = function (ChurchCalendarContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ChurchCalendarSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ChurchCalendarMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ChurchCalendarContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ChurchCalendarMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ChurchCalendarResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        ChurchCalendarMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ChurchCalendarContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ChurchCalendarSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ChurchCalendarMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ChurchCalendarResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ChurchCalendarSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ChurchCalendarMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ChurchCalendarMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ChurchCalendarMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ChurchCalendarResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ChurchCalendarDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ChurchCalendarResult(['ok' => false])]);
        $this->expectException(ChurchCalendarError::class);
        ChurchCalendarDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ChurchCalendarResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ChurchCalendarMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ChurchCalendarMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ChurchCalendarBaseFeature();
        $b = new ChurchCalendarBaseFeature();
        ChurchCalendarFeatureAdd::call($ctx, $a);
        ChurchCalendarFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ChurchCalendarBaseFeature
    {
        $f = new ChurchCalendarBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ChurchCalendarFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ChurchCalendarContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ChurchCalendarSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ChurchCalendarPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        ChurchCalendarPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ChurchCalendarResponse(['headers' => null]),
            'result' => new ChurchCalendarResult([]),
        ]);
        ChurchCalendarResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        ChurchCalendarResultBody::call($ctx);
