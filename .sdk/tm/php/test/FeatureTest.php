...
// ChurchCalendar SDK feature test
//
...
class FtCtrl extends ChurchCalendarControl
{
...
class FtClient
{
    ...
    private ChurchCalendarUtility $utility;

    public function __construct(ChurchCalendarUtility $utility, array $options)
    {
    ...
    public function get_utility(): ChurchCalendarUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ChurchCalendarContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ChurchCalendarContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ChurchCalendarContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ChurchCalendarConfig::make_feature($name);
    ...
    private static function buildUrl(ChurchCalendarSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ChurchCalendarError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ChurchCalendarResponse($fetched);
        }
    ...
    private function fail(ChurchCalendarContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ChurchCalendarContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
