...
// ChurchCalendar SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ChurchCalendarTestFeature extends ChurchCalendarBaseFeature
{
    ...
    public function init(ChurchCalendarContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ChurchCalendarContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ChurchCalendarParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ChurchCalendarContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ChurchCalendarContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ChurchCalendarParam::call($ctx, $k);
