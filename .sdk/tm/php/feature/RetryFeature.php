...
// ChurchCalendar SDK retry feature
...
class ChurchCalendarRetryFeature extends ChurchCalendarBaseFeature
{
    ...
    public function init(ChurchCalendarContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ChurchCalendarContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ChurchCalendarContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ChurchCalendarContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
