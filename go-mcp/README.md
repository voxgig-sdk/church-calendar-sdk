...
[MCP](https://modelcontextprotocol.io) server exposing the ChurchCalendar SDK as
two agent tools — `church-calendar_list` and `church-calendar_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/church-calendar-mcp)
make build

# 2. Provide credentials via the environment
export CHURCH_CALENDAR_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user church-calendar \
  -- /absolute/path/to/church-calendar-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./church-calendar-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// church-calendar_list: first page of records
{ "entity": "calendar" }
{ "entity": "calendar", "query": { } }

// church-calendar_load: one record by id
{ "entity": "calendar", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/church-calendar-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user church-calendar \
     -- "$PWD"/dist/*/church-calendar-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `church-calendar_list` and `church-calendar_load` tools now appear
   in new sessions. Ask the agent to *"list calendar using church-calendar"*
   and it calls `church-calendar_list` with `{"entity":"calendar"}`.
...
```sh
export CHURCH_CALENDAR_APIKEY=sk_live_xxx            # API key
export CHURCH_CALENDAR_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `church-calendar_list` tool
...
### Call the `church-calendar_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `church-calendar_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `church-calendar_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CHURCH_CALENDAR_APIKEY` | API key sent with every request. |
| `CHURCH_CALENDAR_BASE` | Optional override of the API base URL. |
...
```sh
./church-calendar-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"church-calendar_load","arguments":{"entity":"calendar","query":{"id":1}}}}'
```
