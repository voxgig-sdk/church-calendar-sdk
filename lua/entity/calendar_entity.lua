...
CalendarEntity.__index = CalendarEntity


function CalendarEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CalendarEntity)
  self._name = "calendar"
  self._client = client
...
function CalendarEntity:get_name()
  return self._name
end


function CalendarEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CalendarEntity.new(self._client, opts)
end
...
function CalendarEntity:mark_deleted()
  self._deleted = true
end


function CalendarEntity:deleted()
  return true == self._deleted
end


function CalendarEntity:data_set(args)
  if args ~= nil then
...
function CalendarEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CalendarEntity:match_set(args)
  if args ~= nil then
...
function CalendarEntity:match_get()
...
function CalendarEntity:stream(action, args, callopts)
...
---@param reqmatch CalendarLoadMatch
---@param ctrl? table
---@return Calendar
---@return string? err
function CalendarEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CalendarEntity:_run_op(ctx, post_done)
  local utility = self._utility
