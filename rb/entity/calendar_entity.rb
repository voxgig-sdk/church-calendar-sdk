...
class CalendarEntity
  ...
  def make
    opts = @_entopts.dup
    CalendarEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ChurchCalendarHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Calendar, Hash] the current Calendar data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ChurchCalendarHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Calendar fields)
  def match_get
  ...
  #
  # @param reqmatch [CalendarListMatch, Hash, nil] match filter (any subset of
  #   Calendar fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Calendar>, Array] the matching Calendar items; raises ChurchCalendarError on failure
