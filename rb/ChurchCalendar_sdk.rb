...
require_relative 'ChurchCalendar_types'
...
class ChurchCalendarSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ChurchCalendarHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ChurchCalendarHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ChurchCalendarFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ChurchCalendarUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ChurchCalendarHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ChurchCalendarHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ChurchCalendarHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ChurchCalendarError.new(
        "#{op}_allow",
        "ChurchCalendarSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue ChurchCalendarError => err
    ...
    if fetched.is_a?(Hash)
      status = ChurchCalendarHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ChurchCalendarError.new(
        "graphql_error", "ChurchCalendarSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ChurchCalendarSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
