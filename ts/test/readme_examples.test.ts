...
//     (\`new ChurchCalendarSDK(...)\` / \`ChurchCalendarSDK.test(...)\` become
//     \`ChurchCalendarSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ChurchCalendarSDK'
...
const TEST_SEED = {"entity":{"calendar":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ChurchCalendarSDK(...)\` and any
// \`ChurchCalendarSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ChurchCalendarSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ChurchCalendarSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
